	...
	const gint bit_offset, gint no_of_chars)
{
	...
	return (char*)get_ts_23_038_7bits_string(scope, ptr, bit_offset, no_of_chars);
}
			     ...
			     const gint length, const guint encoding)
{
	...
	case ENC_UCS_4:
		strptr = (unsigned char*)tvb_get_ucs_4_string(scope, tvb, offset, length,
		    encoding & ENC_LITTLE_ENDIAN);
		break;
	...
	case ENC_3GPP_TS_23_038_7BITS:
		{
			...
			strptr = (unsigned char*)tvb_get_ts_23_038_7bits_string(scope, tvb, bit_offset, no_of_chars);
		}
		break;
	...
	case ENC_ASCII_7BITS:
		{
			gint bit_offset  = offset << 3;
			gint no_of_chars = (length << 3) / 7;
			strptr = (unsigned char*)tvb_get_ascii_7bits_string(scope, tvb, bit_offset, no_of_chars);
		}
		break;
...
tvb_get_stringz_enc(wmem_allocator_t *scope, tvbuff_t *tvb, const gint offset, gint *lengthp, const guint encoding)
{
	...
	case ENC_UTF_8:
		 ...
		 * CHARACTERs.
		 */
		strptr = (unsigned char*)tvb_get_utf_8_stringz(scope, tvb, offset, lengthp);
		break;

	case ENC_UTF_16:
		strptr = (unsigned char*)tvb_get_utf_16_stringz(scope, tvb, offset, lengthp,
		    encoding & ENC_LITTLE_ENDIAN);
		break;

	case ENC_UCS_2:
		strptr = (unsigned char*)tvb_get_ucs_2_stringz(scope, tvb, offset, lengthp,
		    encoding & ENC_LITTLE_ENDIAN);
		break;

	case ENC_UCS_4:
		strptr = (unsigned char*)tvb_get_ucs_4_stringz(scope, tvb, offset, lengthp,
		    encoding & ENC_LITTLE_ENDIAN);
		break;
